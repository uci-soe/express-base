...
const addressParser = transport.addressParser;
...
module.exports = {
  ...
  send:         function (options) {
    ...
    return Promise.resolve(options)
      ...
      .then(mail => {
        return db.Email.createFromMail(mail)
          .then(saved => dbId = saved._id) // needed to share a similar scope with Email.updateEnvelope to confer which record to update
          .then(() => mail)
      })
      .then(sendMailProm)
      .then(envelope => db.Email.updateEnvelope(dbId, envelope))
      ;
  },

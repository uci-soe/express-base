        ...
        return db.User.findByNetId(auth.resp.ucinetid)
          .then(usr => {
            if (usr) {
              req.user = usr;
              if (!usr.campusid) {
                usr.campusid = +auth.resp.campus_id;
                usr.save();
              }

            // // All users must log in, If they have an account, they are a user, else they are just logged in.
            // } else {
...
module.exports.isAdmin = (req, res, next) => {
  if (req.user && req.user.rights && req.user.rights.isAdmin) {
    return next();
  }
  let err    = new Error('Permission Denied.');
  err.status = 403;
